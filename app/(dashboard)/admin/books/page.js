...
export default function ManageBooks() {
  ...
  const [newBook, setNewBook] = useState({ title: '', author: '', category: '', status: 'Available', coverImage: '', quantity: 1 });
  ...
  const handleAddBook = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setNewBook({ title: '', author: '', category: '', status: 'Available', coverImage: '', quantity: 1 });
        setShowForm(false);
        fetchBooks();
      } else {
